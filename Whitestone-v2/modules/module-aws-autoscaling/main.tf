...
resource "aws_launch_configuration" "this" {
  ...
  name        = var.lc_use_name_prefix == true ? null : local.lc_name
  name_prefix = var.lc_use_name_prefix == true ? "${local.lc_name}-" : null
...
resource "aws_launch_template" "this" {
  ...
  name        = var.lt_use_name_prefix == true ? null  : local.lt_name
...
resource "aws_autoscaling_group" "this" {
  ...
  name        = var.use_name_prefix == true ? null : var.name
  name_prefix = var.use_name_prefix == true ? "${var.name}-" : null
...
resource aws_autoscaling_policy policy_predictive {
  count = var.create_predictive_asg_policy ? 1 : 0
  # count = var.create_asg && var.create_asg_policy ? 1 : 0

  name = "predictive_scaling_policy"
  policy_type = "PredictiveScaling"
  autoscaling_group_name = aws_autoscaling_group.this[0].name

  predictive_scaling_configuration {
    # for_each = lookup(override.value, "predictive_scaling_config", null) != null ? override.value.predictive_scaling_config : [] 
    metric_specification {
      target_value = 35
      predefined_scaling_metric_specification {
        predefined_metric_type = "ASGAverageCPUUtilization"
        resource_label = "${aws_autoscaling_group.this[0].name}-scaling-metric"
      }
      predefined_load_metric_specification {
        predefined_metric_type = "ASGTotalCPUUtilization"
        resource_label = "${aws_autoscaling_group.this[0].name}-load-metric"
      }
    }
...
resource "aws_autoscaling_policy" "scale_up" {
  ...
  name                   = "${aws_autoscaling_group.this[0].name}-scale-up"
...
resource "aws_autoscaling_policy" "scale_down" {
  count                  = local.autoscaling_enabled ? 1 : 0
  name                   = "${aws_autoscaling_group.this[0].name}-scale-down"
...
resource "aws_autoscaling_policy" "scale_up_mem" {
  count                  = local.autoscaling_enabled_mem ? 1 : 0
  name                   = "${aws_autoscaling_group.this[0].name}-scale-up-mem"
  scaling_adjustment     = var.scale_up_scaling_adjustment
  adjustment_type        = var.scale_up_adjustment_type
  policy_type            = var.scale_up_policy_type
  cooldown               = var.scale_up_cooldown_seconds
...
resource "aws_autoscaling_policy" "scale_down_mem" {
  count                  = local.autoscaling_enabled_mem ? 1 : 0
  name                   = "${aws_autoscaling_group.this[0].name}-scale-down-mem"
  scaling_adjustment     = var.scale_down_scaling_adjustment
  adjustment_type        = var.scale_down_adjustment_type
  policy_type            = var.scale_down_policy_type
  cooldown               = var.scale_down_cooldown_seconds
